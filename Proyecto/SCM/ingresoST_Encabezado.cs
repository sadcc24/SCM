...
namespace SCM
{
    public partial class ingresoST_Encabezado : Form
    {
        ...
        {
            if (txtIdMovimiento.Text == "")
            {
                MessageBox.Show("No ha seleccionada ningun registro");
            }
            else
            {
                          ...
                          == DialogResult.Yes)
                {
                    ...
                    int movimiento = Convert.ToInt32(txtIdMovimiento.Text.Trim());
                    new BO.Detalle_ST_BO().eliminaEncabezado(movimiento);
